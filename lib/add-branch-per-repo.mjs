...
export const  addBranchPerRepo = async (destinationDir, repos) => {
    ...
    const remotesList = remotesStr.toString().trim().split('\n').map(s => s.trim())
    ...
    await walkRepos(repos, async (repo, name) => {
        ...
        if (!relevantRemote) {
            return console.log(chalk.red(`Found no relevant branch (main or master) fro ${repo}`));
        }
        await cd(destinationDir)
        ...
        await $`git pull ${relevantRemote.split('/')} --no-commit --allow-unrelated-histories`
